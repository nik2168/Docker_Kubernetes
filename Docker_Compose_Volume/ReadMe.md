...
1. First create a `docker-compose.yaml` file.
2. The app we want to compose must have a `Dockerfile` in it.
3. You can add a `Dockerfile` with the command palette (cmd + shift + p) and then select "Add Docker Files to Workspace Folder".
4. If you added the `Dockerfile` manually, run `docker build -t imageName` to build the image.
5. Don't forget to add docker ignore if adding things manually
...
`docker compose down`
# This command removes all the composed services and their containers.
